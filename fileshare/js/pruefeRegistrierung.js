...
formular.onsubmit=function(){
	...
	if((benutzername.length==0)||(passwort.length==0)||(passwortBestaetigt.length==0)||(email.length==0)){
		fehlerNachricht(fehlerListe, "fehler", 'Alle Felder müssen ausgefüllt sein.');
	...
	if(benutzernameInput.value.length<minBenutzernamenLaenge){
		fehlerNachricht(fehlerListe, "fehler", 'Der Benutzername muss mindestens '+minBenutzernamenLaenge+' Zeichen lang sein.');
		okay = false; // Wenn nicht, abbruch
	}
